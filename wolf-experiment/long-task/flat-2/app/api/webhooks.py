"""Домен webhooks: эндпоинты на swiftframe (мигрировано с miniframe)."""
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена webhooks."""
    ...
    def create(req: sf.Request) -> sf.Reply:
        ...
        data = validate(req.body, FIELDS_WEBHOOKS, partial=False)
    ...
    def patch_one(req: sf.Request) -> sf.Reply:
        ...
        changes = validate(req.body, FIELDS_WEBHOOKS, partial=True)
