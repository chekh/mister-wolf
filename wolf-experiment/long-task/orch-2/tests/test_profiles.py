"""Интеграционные тесты домена profiles (фреймворк-агностичные: только handle())."""
...
def valid_body(suffix: str = "x1") -> dict:
    return {
        "login": f"@profiles-login-{suffix}",
        "display": f"@profiles-display-{suffix}",
        "karma": 7,
        "plan": f"@profiles-plan-{suffix}",
        "badges": 7,
    }
...
class TestProfiles(unittest.TestCase):
    def test_create_ok(self) -> None:
        status, payload = call("POST", "/profiles", valid_body())
    ...
    def test_create_missing_field_400(self) -> None:
        body = valid_body()
        body.pop("login")
        status, payload = call("POST", "/profiles", body)
        self.assertEqual(status, 400)
        self.assertEqual(payload["error"], "validation")

    def test_create_wrong_type_400(self) -> None:
        body = valid_body()
        body["karma"] = "not-an-int"
        status, _ = call("POST", "/profiles", body)
        self.assertEqual(status, 400)

    def test_create_conflict_409(self) -> None:
        call("POST", "/profiles", valid_body("dup"))
        status, payload = call("POST", "/profiles", valid_body("dup"))
        self.assertEqual(status, 409)
        self.assertEqual(payload["error"], "conflict")

    def test_get_ok(self) -> None:
        status, payload = call("GET", "/profiles/1")
        self.assertEqual(status, 200)
        self.assertEqual(payload["id"], 1)

    def test_get_missing_404(self) -> None:
        status, payload = call("GET", "/profiles/99999")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")

    def test_patch_ok(self) -> None:
        status, payload = call("PATCH", "/profiles/2", {"login": f"patched-1"})
        self.assertEqual(status, 200)
        self.assertIn("updated_at", payload)

    def test_patch_invalid_400(self) -> None:
        status, _ = call("PATCH", "/profiles/2", {"karma": "not-an-int"})
        self.assertEqual(status, 400)
    ...
    def test_patch_empty_body_ok(self) -> None:
        status, payload = call("PATCH", "/profiles/1", {})
        self.assertEqual(status, 200)
        self.assertIn("revision", payload)

    def test_get_non_numeric_id_404(self) -> None:
        status, payload = call("GET", "/profiles/not-a-number")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")
