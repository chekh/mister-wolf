"""Интеграционные тесты домена shipments (фреймворк-агностичные: только handle())."""
...
def valid_body(suffix: str = "x1") -> dict:
    return {
        "tracking": f"@shipments-tracking-{suffix}",
        "carrier": f"@shipments-carrier-{suffix}",
        "weight": 7,
        "mode": f"@shipments-mode-{suffix}",
        "parcels": 7,
    }
...
class TestShipments(unittest.TestCase):
    def test_create_ok(self) -> None:
        status, payload = call("POST", "/shipments", valid_body())
    ...
    def test_create_missing_field_400(self) -> None:
        body = valid_body()
        body.pop("tracking")
        status, payload = call("POST", "/shipments", body)
        self.assertEqual(status, 400)
        self.assertEqual(payload["error"], "validation")

    def test_create_wrong_type_400(self) -> None:
        body = valid_body()
        body["weight"] = "not-an-int"
        status, _ = call("POST", "/shipments", body)
        self.assertEqual(status, 400)

    def test_create_conflict_409(self) -> None:
        call("POST", "/shipments", valid_body("dup"))
        status, payload = call("POST", "/shipments", valid_body("dup"))
        self.assertEqual(status, 409)
        self.assertEqual(payload["error"], "conflict")

    def test_get_ok(self) -> None:
        status, payload = call("GET", "/shipments/1")
        self.assertEqual(status, 200)
        self.assertEqual(payload["id"], 1)

    def test_get_missing_404(self) -> None:
        status, payload = call("GET", "/shipments/99999")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")

    def test_patch_ok(self) -> None:
        status, payload = call("PATCH", "/shipments/2", {"tracking": f"patched-1"})
        self.assertEqual(status, 200)
        self.assertIn("updated_at", payload)

    def test_patch_invalid_400(self) -> None:
        status, _ = call("PATCH", "/shipments/2", {"weight": "not-an-int"})
        self.assertEqual(status, 400)
    ...
    def test_patch_empty_body_ok(self) -> None:
        status, payload = call("PATCH", "/shipments/1", {})
        self.assertEqual(status, 200)
        self.assertIn("revision", payload)

    def test_get_non_numeric_id_404(self) -> None:
        status, payload = call("GET", "/shipments/not-a-number")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")
