"""Интеграционные тесты домена inventory (фреймворк-агностичные: только handle())."""
...
def valid_body(suffix: str = "x1") -> dict:
    return {
        "sku": f"@inventory-sku-{suffix}",
        "warehouse": f"@inventory-warehouse-{suffix}",
        "qty": 7,
        "bin": f"@inventory-bin-{suffix}",
        "reserved": 7,
    }
...
class TestInventory(unittest.TestCase):
    def test_create_ok(self) -> None:
        status, payload = call("POST", "/inventory", valid_body())
    ...
    def test_create_missing_field_400(self) -> None:
        body = valid_body()
        body.pop("sku")
        status, payload = call("POST", "/inventory", body)
    ...
    def test_create_wrong_type_400(self) -> None:
        body = valid_body()
        body["qty"] = "not-an-int"
        status, _ = call("POST", "/inventory", body)
        self.assertEqual(status, 400)

    def test_create_conflict_409(self) -> None:
        call("POST", "/inventory", valid_body("dup"))
        status, payload = call("POST", "/inventory", valid_body("dup"))
        self.assertEqual(status, 409)
        self.assertEqual(payload["error"], "conflict")

    def test_get_ok(self) -> None:
        status, payload = call("GET", "/inventory/1")
        self.assertEqual(status, 200)
        self.assertEqual(payload["id"], 1)

    def test_get_missing_404(self) -> None:
        status, payload = call("GET", "/inventory/99999")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")

    def test_patch_ok(self) -> None:
        status, payload = call("PATCH", "/inventory/2", {"sku": f"patched-1"})
        self.assertEqual(status, 200)
        self.assertIn("updated_at", payload)

    def test_patch_invalid_400(self) -> None:
        status, _ = call("PATCH", "/inventory/2", {"qty": "not-an-int"})
        self.assertEqual(status, 400)
    ...
    def test_patch_empty_body_ok(self) -> None:
        status, payload = call("PATCH", "/inventory/1", {})
        self.assertEqual(status, 200)
        self.assertIn("revision", payload)

    def test_get_non_numeric_id_404(self) -> None:
        status, payload = call("GET", "/inventory/not-a-number")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")
