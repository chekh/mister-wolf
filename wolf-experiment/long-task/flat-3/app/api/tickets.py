"""Домен tickets: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_TICKETS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_TICKETS, partial=False)
    if find_by(DOMAIN, "num", data["num"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate num={data['num']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_TICKETS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена tickets (явная регистрация swiftframe)."""
