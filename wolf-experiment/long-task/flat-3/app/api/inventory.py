"""Домен inventory: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_INVENTORY, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_INVENTORY, partial=False)
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_INVENTORY, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена inventory (явная регистрация swiftframe)."""
