"""Домен products: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_PRODUCTS, partial=False)
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_PRODUCTS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена products (явная регистрация swiftframe)."""
