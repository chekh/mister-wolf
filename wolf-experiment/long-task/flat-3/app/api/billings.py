"""Домен billings: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_BILLINGS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_BILLINGS, partial=False)
    if find_by(DOMAIN, "invoice", data["invoice"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate invoice={data['invoice']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_BILLINGS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена billings (явная регистрация swiftframe)."""
