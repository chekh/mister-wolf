"""Домен notifications: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_NOTIFICATIONS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_NOTIFICATIONS, partial=False)
    if find_by(DOMAIN, "topic", data["topic"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate topic={data['topic']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_NOTIFICATIONS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена notifications (явная регистрация swiftframe)."""
