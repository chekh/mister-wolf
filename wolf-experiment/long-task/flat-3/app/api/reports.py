"""Домен reports: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_REPORTS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_REPORTS, partial=False)
    if find_by(DOMAIN, "code", data["code"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate code={data['code']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_REPORTS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена reports (явная регистрация swiftframe)."""
