"""Домен users: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_USERS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_USERS, partial=False)
    if find_by(DOMAIN, "email", data["email"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate email={data['email']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_USERS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена users (явная регистрация swiftframe)."""
