"""Домен searches: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_SEARCHES, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_SEARCHES, partial=False)
    if find_by(DOMAIN, "query", data["query"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate query={data['query']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_SEARCHES, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена searches (явная регистрация swiftframe)."""
