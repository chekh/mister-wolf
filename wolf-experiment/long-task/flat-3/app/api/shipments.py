"""Домен shipments: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_SHIPMENTS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_SHIPMENTS, partial=False)
    if find_by(DOMAIN, "tracking", data["tracking"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate tracking={data['tracking']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_SHIPMENTS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена shipments (явная регистрация swiftframe)."""
