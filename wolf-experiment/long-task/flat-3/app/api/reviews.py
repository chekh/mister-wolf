"""Домен reviews: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_REVIEWS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_REVIEWS, partial=False)
    if find_by(DOMAIN, "ref", data["ref"]) is not None:
        raise ConflictError(DOMAIN, f"duplicate ref={data['ref']}")
    stamp = now()
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_REVIEWS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена reviews (явная регистрация swiftframe)."""
