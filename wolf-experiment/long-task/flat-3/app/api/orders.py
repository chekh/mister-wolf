"""Домен orders: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_ORDERS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_ORDERS, partial=False)
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_ORDERS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена orders (явная регистрация swiftframe)."""
