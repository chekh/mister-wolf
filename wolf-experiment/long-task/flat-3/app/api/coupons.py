"""Домен coupons: эндпоинты на swiftframe (мигрирован с miniframe)."""
...
from app.utils.validation import FIELDS_COUPONS, validate
...
def create(req: sf.Request) -> sf.Reply:
    ...
    data = validate(body, FIELDS_COUPONS, partial=False)
...
def patch_one(req: sf.Request) -> sf.Reply:
    ...
    changes = validate(body, FIELDS_COUPONS, partial=True)
...
def register(app: sf.SwiftApp) -> None:
    """Регистрирует маршруты домена coupons (явная регистрация swiftframe)."""
