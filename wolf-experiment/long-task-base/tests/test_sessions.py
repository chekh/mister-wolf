"""Интеграционные тесты домена sessions (фреймворк-агностичные: только handle())."""
...
def valid_body(suffix: str = "x1") -> dict:
    return {
        "token": f"@sessions-token-{suffix}",
        "user": f"@sessions-user-{suffix}",
        "ttl": 7,
        "scope": f"@sessions-scope-{suffix}",
        "refreshes": 7,
    }
...
class TestSessions(unittest.TestCase):
    def test_create_ok(self) -> None:
        status, payload = call("POST", "/sessions", valid_body())
    ...
    def test_create_missing_field_400(self) -> None:
        body = valid_body()
        body.pop("token")
        status, payload = call("POST", "/sessions", body)
    ...
    def test_create_wrong_type_400(self) -> None:
        body = valid_body()
        body["ttl"] = "not-an-int"
        status, _ = call("POST", "/sessions", body)
        self.assertEqual(status, 400)

    def test_create_conflict_409(self) -> None:
        call("POST", "/sessions", valid_body("dup"))
        status, payload = call("POST", "/sessions", valid_body("dup"))
        self.assertEqual(status, 409)
        self.assertEqual(payload["error"], "conflict")

    def test_get_ok(self) -> None:
        status, payload = call("GET", "/sessions/1")
        self.assertEqual(status, 200)
        self.assertEqual(payload["id"], 1)

    def test_get_missing_404(self) -> None:
        status, payload = call("GET", "/sessions/99999")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")

    def test_patch_ok(self) -> None:
        status, payload = call("PATCH", "/sessions/2", {"token": f"patched-1"})
        self.assertEqual(status, 200)
        self.assertIn("updated_at", payload)

    def test_patch_invalid_400(self) -> None:
        status, _ = call("PATCH", "/sessions/2", {"ttl": "not-an-int"})
        self.assertEqual(status, 400)
    ...
    def test_patch_empty_body_ok(self) -> None:
        status, payload = call("PATCH", "/sessions/1", {})
        self.assertEqual(status, 200)
        self.assertIn("revision", payload)

    def test_get_non_numeric_id_404(self) -> None:
        status, payload = call("GET", "/sessions/not-a-number")
        self.assertEqual(status, 404)
        self.assertEqual(payload["error"], "not_found")
