...
| Опция               | Описание                                                                                                           |
| ------------------- | ------------------------------------------------------------------------------------------------------------------ |
| `--view <view>`     | Выборка: `memory`, `tools`, `rules`, `funnel`, `agents`, `steward`, `outliers`, `readiness`, `all` (дефолт: `all`) |
...
Отфильтровать можно через `--class`, например `--class dead` — кандидаты на archive:
...
Замечание: фильтры `--class`/`--type`/`--origin`/`--agent`/`--silent` сейчас применяются только к `--json`-выводу: текстовый рендер печатает секцию без фильтрации (sleeper в первой строке выше — реальный вывод, не опечатка). `--top` работает в обоих режимах.
...
| Опция         | Описание                                                                                                                                                                                                                                   |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `--tab <tab>` | Одна секция: `health` (L1-статусы, абсолюты, воронка текущего периода), `ledgers` (L2-таблицы: memory, tools, rules, agents, top-N), `trends` (L3-спарклайны по снапшотам, недельная воронка, cache-hit ratio, готовность к экспериментам) |
