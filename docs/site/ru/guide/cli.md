...
Бинарь `wolf` глобального пакета `mister-wolf`. У каждой команды есть `-h, --help`; дословный вывод — `wolf <cmd> --help`. Общие конвенции: `--created-by <actor>` — автор мутации (дефолт: env `WOLF_ACTOR`, иначе `user:cli`); `--tags`/`--applies-to` и прочие списки — через запятую.
