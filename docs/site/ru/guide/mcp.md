...
Каноническая команда запуска — глобальный бинарь, **никогда npx**:

```json
{ "command": "wolf", "args": ["mcp"] }
```

**opencode** → `opencode.json`, ключ `mcp.wolf`:

```json
{ "mcp": { "wolf": { "type": "local", "command": ["wolf", "mcp"], "enabled": true } } }
```

**Claude Code** → `.mcp.json`, ключ `mcpServers.wolf`:
