...
- [ ] **Step 4.4: Run → FAIL → реализация get-call-injections.ts**: сигнатура `getCallInjections(deps {store, index?, clock}, input {topic?, thread?, compact?}) → блоки вывода` — deps унифицированы с buildSolvePack (Step 3.8): index optional, clock для now в recencyFactor; filter type=call-injection status=active; topic-match через trigger_keywords ∩ topic-слова, иначе FTS; fallback rules ≤3; бюджет 1200 символов, обрезка целыми блоками; провенанс обязателен. → GREEN.
