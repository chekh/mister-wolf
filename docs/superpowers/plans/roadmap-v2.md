...
- LLM-тегирование `error_class_id` — опциональное уточнение границ кластеров через адаптер `opencode run` (инвариант local-first записи сохраняется: эмбеддинги считаются при детекции, не при логировании).
