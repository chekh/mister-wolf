...
| --------------- | ------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Память**      | Единая точка правды: объекты (25 активных типов, включая `complaint`), supersede-цепочки версий, relations, атрибуция actor    | `src/domain/` (сущности, `taxonomy.ts`, `governance.ts`), `src/adapters/fs/markdown-memory-store.ts` (Markdown-канон), `src/adapters/sqlite/` (FTS-индекс), `.wolf/memory/` |
