...
noise: 190/247 = 76.9% [BAD]
documents: 122 (registered refs, не участвуют в метрике шума) [INFO]
archived: 15 (вне метрики шума) [INFO]
routing: zai-coding-plan/glm-5.2: tasks=3 median=22868.2
totals: runs=2 failures=0 weighted=42736 cache=n/a avg=n/a
cost: n/a (pricing не настроен)
...
| `totals` / `cost` / `model` | run-сигналы `.wolf/metrics/session-metrics.jsonl` (M3; НЕ run-log) | блок абсолютов: runs и failures (`outcome ≠ ok`), суммы weighted и raw-токенов, cache-hit ratio `cache_read/(input+cache_read)`, средняя duration_ms; по моделям — runs/failures и cost-per-success (`$cost/(runs−failures)`). `$`-поля только при `pricing` в конфиге (см. [analytics.md](./analytics.md)), иначе честное `n/a (pricing не настроен)`; `n/a` также пока не накоплены M1-данные (записи без `tokens`/`duration_ms`). Информативный |
