# MEMORY.md — Правила работы с памятью Mr. Wolf (архив: docs/archive/MEMORY.md)

> **АРХИВ (archived 2026-08-25):** контент перенесён в память Wolf (`.wolf/memory`), файл устарел и не поддерживается. Актуальные правила — `wolf brief` / `wolf search`.
...
После завершения каждого этапа работы агент обязан обновить проектную документацию: `AGENTS.md`, `README.md`, `MEMORY.md` (этот файл, заархивирован → docs/archive/MEMORY.md) и любые затронутые документы в `docs/`. Это правило имеет приоритет над спешкой и не может быть пропущено. _(Правило устарело и заменено: см. rule «Единственный источник состояния — память Wolf» в памяти Wolf.)_
