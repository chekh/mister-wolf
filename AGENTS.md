...
- Solve/call: scenario-classified Solve Pack + compact call injections, deterministic (no LLM). `wolf solve` is read-only; `--save` creates an info-request. When the user says "listen to Wolf" / "слушай Wolf", run `wolf call` and treat returned injections as active project guidance.
...
- Phase 9: Solve/Call — `wolf solve` (scenario-classified Solve Pack, `--save` for memory repair info-request), `wolf call` (compact call injections with topic matching, thread mode, budget), `wolf relation add` to link injections.

## Next phase

- Decide from roadmap-v2.
...
2. Run `npm run check` before and after changes.
3. After completing any phase, update AGENTS.md, README.md, MEMORY.md, and affected docs.
