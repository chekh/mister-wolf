...
export function validateMemoryObject(obj: MemoryObject): ValidationResult {
  ...
  if (obj.tags.length === 0 && Object.keys(obj.related).every((k) => obj.related[k as keyof typeof obj.related].length === 0)) {
    warnings.push('No tags or related links; memory may be hard to discover.');
  }

  const hasMeaningfulContent =
    obj.body.trim().length > 20 ||
    obj.tags.length > 0 ||
    Object.values(obj.related).some((arr) => arr.length > 0);
